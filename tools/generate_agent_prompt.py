...
по JA/EN, а не по внешнему виду. Для томов со смысловыми блоками
(маркеры `<!-- block: N -->`) абзацную структуру RU внутри блока выбираешь
сам, границу блока не переносить; для старых томов сохраняй построчное
соответствие EN (alignment-инвариант). Курсив — только *…*.
...
(для старых томов — число абзацев следует EN-файлу).
...
- Для томов со смысловыми блоками: граница блока (`<!-- block: N -->`) —
  alignment-инвариант, её не переносить; абзацы внутри блока можно
  перестраивать, сохраняя смысл JA.
  Для старых томов: число и порядок абзацев — alignment-инвариант EN,
  абзац не разбивать и не склеивать при правке оформления.
  Сверка — scripts/check_alignment.py.
