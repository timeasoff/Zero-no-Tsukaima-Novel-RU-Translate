...
поэтому расхождение абзацной разбивки между языками не смещает контекст
дальше по главе.
...
def _soft_boundary(prev, nxt, narr_max):
    ...
    Блок укрупнён: наррация склеивается с наррацией, реплики — с репликами
    и окружающей их наррацией; жёстких границ внутри текста почти нет —
    размер блока ограничивают max_paras/max_chars (см. cluster_blocks).
    """
    ...
    if is_speech(nxt) and len(prev) <= narr_max:
        return True
    # 4) наррация → наррация: по умолчанию тоже склеиваем (укрупнение блоков)
    return True


DEFAULT_PARAMS = {
    "narr_max": 150,     # порог «короткой» наррации для склейки с репликой
    "max_paras": 16,     # максимум абзацев в одном блоке
    "max_chars": 1600,   # максимум символов в одном блоке
}
